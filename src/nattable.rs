...
pub struct RLHash<LK:Copy+Eq+Hash, C: KeyGen<LK, DefaultKey>>{
    locals: HashMap<LI<LK>, DefaultKey>,  // index of entries with key (depends on NATTable type)
    locals_last_assigned: HashMap<(Ipv4Addr, u16), u16>,  // index of entries with key (depends on NATTable type)
    remotes: HashMap<RI<LK>, DefaultKey>, // index of entries with key (depends on NATTable type)
    keygen: C
}
impl<LK:Display+Copy+Eq+Hash, C: KeyGen<LK, DefaultKey>> RLHash<LK, C> {
    ...
    pub fn get_empty_port(&mut self, p:&PacketInfo) -> Option<u16>{
        ...
        self.keygen.empty_port(&self.remotes, p.dst_tuple, p.src_tuple.1, last_assigned)
    }
...
//for ordinary EIM/ADF and EIM/APDF behavior, we may need "remote mapping key" and "remote filter key", and more complex NatEntry dat
impl<LK:Display+Copy+Eq+Hash, C: KeyGen<LK, DefaultKey>> HashNATTable<LK, C> {
    ...
    pub fn print_status(&self){
        ...
        for (e,_k) in LLIter(&self.entrylist, self.entrylist.head()){
            ...
            println!("{}[{}]{}->{}",newdate, e.global_port, AddrPort::from_t(e.local_tuple), e.nat_key)
        }
    }
    ...
    fn _insert_and_egress<'a>(&mut self, now: u64, p: PacketInfo, global_port: u16, lk:LK) ->  Option<u16> {
        ...
        self.write_log(format!("[{}]{} inserting: via {}: {} -> {}\n", self.name, now, global_port, AddrPort::from_t(p.src_tuple), lk));
